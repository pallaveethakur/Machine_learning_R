#credit card  logistics regression
...
probability_value <- plogis(predict(cc.train.model3,testset[-2]))
probability_value2 <- plogis(predict(cc.train.model4,testset[-2]))

threshold=0.8#by changing threshold
predicted_values<-ifelse(predict(cc.train.model3,type="response")>threshold,1,0)
actual_values<-cc.train.model3$y
...
plot.roc(testset$card,probability_value)

plot.roc(testset$card,prediction3)
par(pty="s")#to get sign lines removed
...
warnings()
plot.roc(testset$card,probability_value2,print.auc=TRUE)



#prediction3 or cc.train.model 3 is final.
